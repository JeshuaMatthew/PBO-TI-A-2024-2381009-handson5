...
public class Main {
    ...
    public static void showTodoList() {
        ...
        for (int i = 0; i < todos.length; i++) {
            String todo = todos[i];
            if (todo != null) System.out.println((i + 1) + ". " + todo);
        }
    }

    public static void addTodoList(String todo) {
        resizeArrayIfFull(todo);
    ...
    private static void resizeArrayIfFull(String todo) {
        boolean isFull = isArrayFull();

        //untuk mendouble banyak memori yang bisa ditampung oleh array todos
        if (isFull) {
    ...
    private static boolean isSelectedTodoNotValid(Integer number) {
        ...
        if (number - 1 > todos.length - 1) return true;

        if (todos[number - 1] == null) return true;
        return false;
    }
